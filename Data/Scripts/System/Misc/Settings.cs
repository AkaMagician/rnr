...
namespace Server.Misc
{
    class MyServerSettings
    {
        ...
        public static void Configure()
        {
			if ( System.IO.File.Exists( "Info/settings.xml" ) )
			{
				...
				if ( root != null )
				{
					...
					{
						if ( setting == 1 ){ S_EnableConsole = bool.Parse(node.InnerText); }
						...
						else if ( setting == 15 ){ S_CreaturesSearching = bool.Parse(node.InnerText); }
						...
						else if ( setting == 54 ){ S_NoMountBuilding = bool.Parse(node.InnerText); }
						else if ( setting == 55 ){ S_NoMountsInHouses = bool.Parse(node.InnerText); }
						else if ( setting == 56 ){ S_AllowCustomTitles = bool.Parse(node.InnerText); }
						else if ( setting == 57 ){ S_AllowHouseDyes = bool.Parse(node.InnerText); }
						else if ( setting == 58 ){ S_AllowCustomHomes = bool.Parse(node.InnerText); }
						else if ( setting == 59 ){ S_AllowCraftMagic = bool.Parse(node.InnerText); }
						else if ( setting == 60 ){ S_Bribery = Int32.Parse(node.InnerText); }
						else if ( setting == 61 ){ S_LineOfSight = bool.Parse(node.InnerText); }
						else if ( setting == 62 ){ S_ChangeArtyLook = bool.Parse(node.InnerText); }
						else if ( setting == 63 ){ S_Resources = Int32.Parse(node.InnerText); }
						else if ( setting == 64 ){ S_SoldResource = bool.Parse(node.InnerText); }
						else if ( setting == 65 ){ S_Humanoids = bool.Parse(node.InnerText); }
						else if ( setting == 66 ){ S_MerchantBooks = bool.Parse(node.InnerText); }
						else if ( setting == 67 ){ S_HouseStorage = bool.Parse(node.InnerText); }
						else if ( setting == 68 ){ S_CorpseDecay = Int32.Parse(node.InnerText); }
						else if ( setting == 69 ){ S_BoneDecay = Int32.Parse(node.InnerText); }
						else if ( setting == 70 ){ S_HouseOwners = bool.Parse(node.InnerText); }
						else if ( setting == 71 ){ S_LawnsAllowed = bool.Parse(node.InnerText); }
						else if ( setting == 72 ){ S_ShantysAllowed = bool.Parse(node.InnerText); }
						else if ( setting == 73 ){ S_MonsterCharacters = Int32.Parse(node.InnerText); }
						else if ( setting == 74 ){ S_DeleteDays = XmlConvert.ToDouble(node.InnerText); }
						else if ( setting == 75 ){ S_AutoAccounts = bool.Parse(node.InnerText); }
						else if ( setting == 76 ){ S_Port = Int32.Parse(node.InnerText); }
						else if ( setting == 77 ){ S_SpecialWeaponAbilSkill = XmlConvert.ToDouble(node.InnerText); }
		...
		{
			if ( S_StatGainDelay > 60 ){ S_StatGainDelay = 60; } else if ( S_StatGainDelay < 5 ){ S_StatGainDelay = 5; }
		...
		{
			if ( S_HousesPerAccount == 0 ){ S_HousesPerAccount = 1; }
			else if ( S_HousesPerAccount < 0 ){ S_HousesPerAccount = -1; }
		...
		{
			if ( S_CorpseDecay < 1 ){ S_CorpseDecay = 0; }
		...
		{
			if ( S_BoneDecay < 1 ){ S_BoneDecay = 0; }
		...
		{
			if ( S_DeleteDays < 1 ){ S_DeleteDays = 0; }
		...
		{
			if ( S_SpecialWeaponAbilSkill < 20 ){ S_SpecialWeaponAbilSkill = 20.0; }
